...
trait CompanyRepository
{
    ...
    public static function mapLeadsData($leads, $averageResponseTime, $startDate, $endDate, $format = 'Y-m-d')
    {
        ...
        $datasets = collect($datasets)->map(function ($dataset) use ($leads, $dateCollection) {
            ...
            $dataset['data'] = collect($dateCollection)->map(function ($date) use ($leads, $fieldName) {
                $value = $leads->where('creation_date', $date)->first();
                return isset($value[$fieldName]) ? (int) $value[$fieldName] : 0;
            });
            return $dataset;
        });
        ...
        (
            ...
            (SELECT lead_notes.lead_id, MIN(lead_notes.created_at) AS created_at
                          ...
                          WHERE
                              ...
                              lead_statuses.type = 'CONTACTED_EMAIL' OR

...
class AgencyController extends Controller
{
    ...
    public function store(Request $request, Agency $agency)
    {
        ...
        if (($event === 'sales') || ($event === 'subscription-created') || ($event === 'subscription-trial-start')) {
            try {
    ...
    private function createCompany(Request $request, $agencyUUID, Company $company): Company
    {
        ...
        $company->createCompany($request->only(['name', 'avatar_id', 'phone', 'email', 'password', 'password_confirmation', 'is_locked']));
    ...
    public function getUserInfo($userid){
        return User::where('id', $userid)->get(['role', 'subscription_type', 'max_domains', 'max_projects'])->first();
    }
}

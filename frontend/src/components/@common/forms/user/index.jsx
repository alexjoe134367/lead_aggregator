...
class UserForm extends Component {
    ...
    onChangeSubscription = (event, data) => {
        ...
        if (data.value === 'BASE' && (this.props.form.max_agency_companies > 5 || !this.props.form.max_agency_companies)) {
            ...
            this.props.changeForm({ max_domains: 5 });
            this.props.changeForm({ max_projects: 5 });
        } else if (data.value === 'PREMIUM' && this.props.form.max_agency_companies < 10) {
            this.props.changeForm({max_agency_companies: 10});
            this.props.changeForm({max_domains: 10});
            this.props.changeForm({max_projects: 10});
        } else {
    ...
    render() {
        const {
            ...
            avatar_path, subscription_type, max_agency_companies, max_projects, max_domains
        } = this.props.form;
                            ...
                            role === 'AGENCY'
                                ?   <Form.Field>
                                        ...
                                        <Input placeholder='Max Agency companies'
                                               ...
                                               type='number'
                                               onChange={this.onChange}/>
                                        <label>Max Agency domains can create</label>
                                        <Input placeholder='Max Agency domains'
                                               name='max_domains'
                                               min={5}
                                               value={max_domains}
                                               type='number'
                                               onChange={this.onChange}/>
                                        <label>Max Agency projects can create</label>
                                        <Input placeholder='Max Agency projects'
                                               name='max_projects'
                                               min={5}
                                               value={max_projects}
                                               type='number'

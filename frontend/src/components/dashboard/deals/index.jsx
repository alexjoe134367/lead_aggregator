...
const zoom = (
  <i className="flaticon stroke zoom-2 icon">
    </i>
);
      ...
      <div className={'Dashboard ' + (dealIds.length ? 'sidebarOpened' : '')}>
        ...
        }
        {
          !visible || <Message className='dash' onDismiss={this.handleDismiss}>
            ...
            <Message.Content>
              <p>Check out our getting started video tutorial below.</p>
              <a className="item" href="https://convertlead.ladesk.com/137942-Getting-Started-with-ConvertLead" target="react/jsx-no-target-blank">Take me there</a>
            </Message.Content>
          </Message>
        }
        <DealModal />
            ...
            <Grid.Column>
              <Menu secondary>
                <Menu.Menu position='right'>
                  <Menu.Item>
                    <Input icon={zoom} onChange={this.searchDealsByCompany} value={filters.search}
                      placeholder='Search...' />
                  </Menu.Item>
                  <Button color='teal'
                    className="new-campaign icon"
                    onClick={this.props.loadForm.bind(this, { show: true })} ><i className='flaticon stroke plus-1 icon'/></Button>
                    </Menu.Menu>
              </Menu>
            </Grid.Column>
          </Grid>
